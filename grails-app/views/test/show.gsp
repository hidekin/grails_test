			...
			<ol class="property-list test">
				...
				<li class="fieldcontain">
					<span id="test-label" class="property-label"><g:message code="test.test.label" default="Test" /></span>
						...
						<g:each in="${testInstance.test}" var="t">
						<span class="property-value" aria-labelledby="test-label"><g:link controller="user" action="show" id="${t.id}">${t?.encodeAsHTML()}</g:link></span>
						</g:each>
