	...
	<body>
		...
		<div id="show-group" class="content scaffold-show" role="main">
			...
			<ol class="property-list group">
				...
				<g:if test="${groupInstance?.subscribers}">
				<li class="fieldcontain">
					<span id="subscribers-label" class="property-label"><g:message code="group.subscribers.label" default="Subscribers" /></span>
					
						<g:each in="${groupInstance.subscribers}" var="s">
						<span class="property-value" aria-labelledby="subscribers-label"><g:link controller="user" action="show" id="${s.id}">${s?.encodeAsHTML()}</g:link></span>
						</g:each>
