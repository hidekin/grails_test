			...
			<table>
				...
				<g:each in="${patientsInstanceList}" status="i" var="patientsInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${patientsInstance.id}">${fieldValue(bean: patientsInstance, field: "firstname").capitalize()}</g:link></td>
					
						<td>${fieldValue(bean: patientsInstance, field: "lastname").capitalize()}</td>
					
						<td>${fieldValue(bean: patientsInstance, field: "address").capitalize()}</td>
					
						<td>${fieldValue(bean: patientsInstance, field: "symptom").capitalize()}</td>
					
						<td>${fieldValue(bean: patientsInstance, field: "description").capitalize()}</td>
					
						<td>${fieldValue(bean: patientsInstance, field: "insurance_number").capitalize()}</td>
